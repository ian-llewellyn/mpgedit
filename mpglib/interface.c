#ifndef lint
static char SccsId[] = "$Id: interface.c,v 1.4 2004/07/19 14:44:10 number6 Exp $";
#endif

/* $Id: interface.c,v 1.4 2004/07/19 14:44:10 number6 Exp $ */
...
BOOL InitMP3( PMPSTR mp) 
{
	...
	mp->synth_bo = 1;
	mp->look_for_xing = 0;
	mp->_sideinfo = (void *) calloc(1, sizeof(struct III_sideinfo));
...
#if !defined(_AIX) 
                   /* 
                    * AIX does not like calling a
                    * function from a shared library 
                    * that is in the application that 
                    * loaded the plugin
                    */
				...
				int *bsbits)
{
	...
	struct III_sideinfo *sideinfo = (struct III_sideinfo *) mp->_sideinfo;
	...
	if(!mp->header_parsed) {
		...
		mp->ssize += 2;
        /*
         * When a previous frame is passed, load the
         * alternate backspace buffer with this frame data.
         */
        if (prev_isize>0 && prev_in) {
            /*
             * adam/TBD: This computation is a hack, should really be done by
             * decode_header().  But all frame headers are 4 bytes, right?
             * This works so long as only a complete frame, and no more are
             * passed in the prev_in argument.  This is true for use by
             * mpgedit.
             */
            mp->fsizeold = prev_isize - 4;
            memcpy(mp->bsspace[mp->bsnum]+512, prev_in+4, mp->fsizeold);
        }
	...
	if (!mp->side_parsed) {
		...
		if(mp->fr.error_protection)
		  getbits(16);
		bits=do_layer3_sideinfo(&mp->fr, sideinfo);
		...
		mp->side_parsed=1;
        if (bsbits) {
            *bsbits = sideinfo->main_data_begin;
        }
	}
...
#endif
			case 3:
				do_layer3(mp,(unsigned char *) out,done,sideinfo);
			break;
			default:
...
int decodeMP3( PMPSTR mp,unsigned char *in,int isize,char *out,
		int osize,int *done)
{
    return decodeMP3_9(mp, in, isize, NULL, 0, out, osize, done, NULL);
}
