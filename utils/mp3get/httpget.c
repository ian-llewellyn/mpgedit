...
void readstringsock (char *string, int maxlen, int sock)
{
	int pos = 0;

	while(1) {
		if( recv(sock, string+pos,1,0) == 1) {
			pos++;
			if(string[pos-1] == '\n') {
				string[pos] = 0;
				break;
...
int http_open (char *url)
{
	...
	do {
		...
		memcpy(&sin.sin_addr, hp->h_addr, hp->h_length);
                sin.sin_port = htons((short int)atoi((char *) myport));
		if (connect(sock,
            (struct sockaddr *)&sin,
...
fail:
		...
		relocate = FALSE;
		purl[0] = '\0';
		readstringsock (request, linelength-1, sock);
