...
if [ -n "$1" ]; then
  dist_dir="../../mpgedit_${1}_macosx"
fi
if [ -z "$dist_dir" ]; then
  echo "ERROR: distribution directory for Mac OS X build not specified"
  exit 1
fi
if [ -d $dist_dir ]; then
...
lib="libdecoder_mpg123.dylib libdecoder_mad.dylib libdecoder_popen.dylib libmpglib_mpgedit.dylib"
...
(cd $dist_dir; ls -l | sed -e '/^total/d' -e "/$README/d" >> $README)
#                                                       README
#-rw-r--r--   1 number6  slocate      3803 Jan 29 15:28 decoder.so.1
#-rwxr-xr-x   1 number6  slocate      3519 Jan 29 17:34 install.sh*
#-rwxr-xr-x   1 number6  slocate     39343 Jan 29 15:28 libdecoder_mpg123.dylib*
#-rwxr-xr-x   1 number6  slocate     16734 Jan 29 15:28 libdecoder_popen.dylib*
#-rwxr-xr-x   1 number6  slocate    139047 Jan 29 15:28 libmpglib_mpgedit.dylib*
...
cat << NNNN >> $dist_dir/$README
      ...
      ------------                    ---------
                                  $README
NNNN
(cd $dist_dir; md5sum * | sed -e "/README.macosx/d" >> $README)
#                                  README
#283742550a52c1170cf6d8ffab488847  decoder.so.1
#4b40fb734e88b87528be82e6dbe72d5e  install.sh
#cec6c602641a3f7c3de9523405504c07  libdecoder_mpg123.dylib
#cec6c602641a3f7c3de9523405504c07  libdecoder_mad.dylib
#29e2e65bb91337a6be299699fe7aabb7  libdecoder_popen.dylib
#4598d35581699c34bbdb5ea77c049190  libmpglib_mpgedit.dylib
...
NNNN
cat $dist_dir/README | \
    sed -n  -e '1,/^================/p' | sed '$,$d' >> $dist_dir/$README
