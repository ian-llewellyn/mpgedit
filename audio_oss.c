...
int oss_gain(struct audio_info_struct *ai)
{
  if(ai->gain >= 0) {
    int e,mask;
    e = ioctl(ai->fn , SOUND_MIXER_READ_DEVMASK ,&mask);
...
#if !defined(OSS_DSP_MIXER_BROKEN) /* Hack for systems with broken PCM mixer */
    else 
#endif
...
int audio_get_gain(struct audio_info_struct *ai)
{
  ...
  if(e < 0) {
    ...
    e = ioctl(ai->fn, SOUND_MIXER_READ_PCM , &gain);
  }
  else if(!(mask & SOUND_MASK_VOLUME)) {
    fprintf(stderr,"audio/gain: setable Volume/PCM-Level not supported by your audio device: %#04x\n",mask);
  }
  else {
    e = ioctl(ai->fn, SOUND_MIXER_READ_VOLUME , &gain);
  }
  if (e != -1 && gain != -1) {
    ai->gain = gain;
  }
